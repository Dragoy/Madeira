// libc++ PMR compatibility override for iOS 16.
//
// libc++ 16 moved std::pmr::memory_resource's virtual destructor into
// libc++.1.dylib. Apple shipped that ABI starting with iOS 17. Building with a
// newer SDK while targeting iOS 16 can therefore produce a launch-time dyld
// failure. Keep the pre-libc++16 behavior by making the destructor inline.
//
// This file intentionally uses libc++'s internal include path/name so placing
// this directory before the SDK include roots overrides only this one header.
...
class _LIBCPP_AVAILABILITY_PMR memory_resource {
  static const size_t __max_align = alignof(max_align_t);

public:
  // libc++ <= 15 behavior: no runtime symbol required.
  _LIBCPP_HIDE_FROM_ABI virtual ~memory_resource() = default;
...
[[nodiscard, __gnu__::__returns_nonnull__]] _LIBCPP_AVAILABILITY_PMR memory_resource*
get_default_resource() noexcept;

[[__gnu__::__returns_nonnull__]] _LIBCPP_AVAILABILITY_PMR memory_resource*
set_default_resource(memory_resource*) noexcept;

[[using __gnu__: __returns_nonnull__, __const__]] _LIBCPP_AVAILABILITY_PMR memory_resource*
